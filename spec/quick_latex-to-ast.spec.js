...
var trees = {
  ...
  "\\frac{d^2\\var{hello}}{d\\var{bye}^2}": [
    "derivative_leibniz",
    ["tuple", "hello", 2],
    ["tuple", ["tuple", "bye", 2]]
  ],
  ...
  "\\frac{\\partial \\var{hello}}{\\partial \\var{bye}}": [
    "partial_derivative_leibniz",
    "hello",
    ["tuple", "bye"]
  ],
  ...
  "\\frac{\\partial ^2\\var{hello}}{\\partial \\var{bye}^2}": [
    "partial_derivative_leibniz",
    ["tuple", "hello", 2],
    ["tuple", ["tuple", "bye", 2]]
  ],
...
test("neq", () => {
  let converter = new latexToAst({ missingFactor: (token, e) => 0 });
  const result = converter.convert("\\neq(b)");
  expect(result).toEqual(["ne", 0, "b"]);
});
test("40% 100", () => {
  let converter = new latexToAst({ missingFactor: (token, e) => 0 });
  const result = converter.convert("40% 100");
  expect(result).toEqual(["*", ["%", 40], 100]);
});
...
test("unknown commands passthrough", function() {
  let converter = new latexToAst({ unknownCommandBehavior: "passthrough" });
  let convert = input => converter.convert(input);

  expect(convert("\\parallel x")).toEqual(["*", "parallel", "x"]);
  expect(convert("\\overrightarrow{x}")).toEqual(["*", "overrightarrow", "x"]);
  expect(convert("\\overleftrightarrow{x}")).toEqual([
    "*",
    "overleftrightarrow",
    "x"
  ]);
  expect(convert("\\perp x")).toEqual(["*", "perp", "x"]);
  expect(convert("x \\angle")).toEqual(["*", "x", "angle"]);
  expect(convert("x \\overarc{x}")).toEqual(["*", "x", "overarc", "x"]);
});

test("throw error by default for unknown commands", () => {
  expect(() => convert("\\parallel x")).toThrow();
});
