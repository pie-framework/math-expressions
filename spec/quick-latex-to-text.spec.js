...
const converter = new me.converters.latexToAstObj({
...
const toText = new me.converters.astToTextObj({
  unicode_operators: {
...
const fromLatex = latex => {
  const ast = converter.convert(latex);
  return toText.convert(ast);
};
...
test("-12.5\\%", () => {
  expect(fromLatex("-12.5%").toString()).toEqual("- 12.5%");
});
...
test("40%", () => {
  const result = fromLatex("40%");
  expect(result.toString()).toEqual("40%");
});
